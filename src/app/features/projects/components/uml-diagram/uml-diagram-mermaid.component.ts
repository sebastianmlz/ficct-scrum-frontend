...
export class UMLDiagramMermaidComponent implements OnInit {
  ...
  ngOnInit(): void {
    console.log('[UML-MERMAID] Component initialized for project:', this.projectId);
  ...
  parseUMLData(response: any): void {
    try {
      ...
      console.log('[UML-MERMAID] Generated Mermaid syntax (first 200 chars):', mermaidSyntax.substring(0, 200));
  ...
  async renderMermaid(): Promise<void> {
    if (!this.mermaidInitialized || !this.mermaidCode() || !this.mermaidContainer) {
      return;
    }
  ...
  setupClickHandlers(): void {
    ...
    const svgElements = this.mermaidContainer.nativeElement.querySelectorAll('.node');
    ...
    console.log('[UML-MERMAID] Click handlers setup for', svgElements.length, 'nodes');
  }
  ...
  loadGitHubIntegration(): void {
    ...
    this.githubService.getIntegration(this.projectId).subscribe({
      next: (integration: any) => {
        ...
        console.log('[UML-MERMAID] GitHub integration:', !!this.gitHubIntegration);
      },
      error: () => {
        this.isLoadingIntegration.set(false);
