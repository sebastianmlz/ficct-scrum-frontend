...
export class SlackWebhookTestComponent {
  ...
  testMessage =
    signal('Hello from FICCT-SCRUM! 👋 This is a test notification.');
  ...
  testWebhook(): void {
    if (!this.webhookUrl() || !this.testMessage()) {
      this.notificationService
          .warning('Please provide both webhook URL and message');
      return;
    }
    ...
    this.notificationsBackendService.testSlackWebhook(
      ...
      next: (result) => {
        ...
        if (result.success) {
          this.notificationService
              .success('Success', 'Test message sent to Slack!');
        } else {
