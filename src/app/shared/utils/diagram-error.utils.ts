 ...
 * Provides consistent error handling and user-friendly messages for all diagram components
 */
 ...
 */
export function analyzeDiagramError(error: any, projectId?: string): DiagramErrorState {
  ...
  if (error.status === 500 && 
    ...
    return {
      ...
      message: 'Start working on issues and sprints to generate insights and visualizations.',
      icon: 'chart-empty',
      canRetry: false
    };
  }
  ...
  // Network Error
  if (error.status === 0 || error.name === 'HttpErrorResponse' && !error.status) {
    return {
      type: 'NETWORK_ERROR',
